// NebulaTransport - TCP transport over Nebula mesh network
// Implements TransportAdapter interface for Nebula-based connectivity
...
/**
 * TCP-based transport adapter for Nebula mesh networks.
 *
 * This transport uses plain TCP sockets over the Nebula virtual network interface.
 * Nebula provides the encrypted tunnel; this adapter handles:
 ...
 * - Peer identity (just uses provided endpoint info)
 */
export class NebulaTransport extends EventEmitter implements TransportAdapter {
  readonly type = 'nebula'

  private config: Required<Pick<NebulaTransportConfig, 'nebulaIp' | 'port' | 'connectionTimeout'>>
  ...
  constructor(config: NebulaTransportConfig) {
    super()
    this.config = {
