// HeadscaleTransport - TCP transport over Headscale mesh network
// Implements TransportAdapter interface for Headscale-based connectivity
...
import { HeadscaleCLI, type HeadscaleConfig, type TailscalePeerInfo } from './cli'
...
/**
 * TCP-based transport adapter for Headscale mesh networks.
 *
 * Headscale is a self-hosted, open-source implementation of the Tailscale
 * control server. This transport uses the standard Tailscale client but
 * configures it to connect to a Headscale server instead.
 *
 * Features:
 * - TCP connections over Tailscale/Headscale virtual network
 * - Same interface as TailscaleTransport
 * - Pre-auth key support for automated node registration
 * - Optional Headscale API integration for node management
 *
 * This transport uses plain TCP sockets over the virtual network interface.
 * The Headscale/Tailscale stack provides the encrypted tunnel.
 */
export class HeadscaleTransport extends EventEmitter implements TransportAdapter {
  readonly type = 'headscale'
  ...
  constructor(config: HeadscaleTransportConfig) {
    super()
    this.config = {
    ...
    const headscaleConfig: HeadscaleConfig = {
      serverUrl: this.config.serverUrl,
      preAuthKey: this.config.preAuthKey,
      apiKey: this.config.apiKey,
      hostname: this.config.hostname,
      tailscaleBin: this.config.tailscaleBin,
    }

    this.cli = new HeadscaleCLI(headscaleConfig)
  }
  ...
  async start(): Promise<void> {
    ...
    // Check Tailscale/Headscale status
    const isConnected = await this.cli.isConnected()
    if (!isConnected) {
      const state = await this.cli.getBackendState().catch(() => 'Unknown')
      throw new Error(
        `Headscale is not connected (state: ${state}). ` +
          `Run 'tailscale up --login-server ${this.config.serverUrl}' first.`
      )
    }
  ...
  // ===========================================================================
  // Headscale-Specific Methods
  // ===========================================================================

  /**
   * Get the underlying Headscale CLI wrapper.
   */
  getCLI(): HeadscaleCLI {
    return this.cli
  }
   ...
   */
  async getHeadscalePeers(): Promise<TailscalePeerInfo[]> {
    return this.cli.getPeers()
  }

  /**
   * Get online peers in the network.
   */
  async getOnlineHeadscalePeers(): Promise<TailscalePeerInfo[]> {
