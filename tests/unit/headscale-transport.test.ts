// Tests for HeadscaleTransport

import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest'
import { HeadscaleTransport, HeadscaleCLI } from '../../src/transports/headscale'
import * as net from 'net'

describe('HeadscaleTransport', () => {
  let transport: HeadscaleTransport

  // Helper to setup CLI mocks on transport instance
  function mockCLI(transport: HeadscaleTransport) {
    ...
    // Use localhost for testing since Headscale IPs aren't available
    ...
    vi.spyOn(cli, 'getStatus').mockResolvedValue({
      ...
      Self: {
        ...
        DNSName: 'test-node.headscale.example.com',
        HostName: 'test-node',
        OS: 'linux',
        UserID: 12345,
      },
  ...
  beforeEach(() => {
    ...
    const port = 18946 + Math.floor(Math.random() * 1000)
    transport = new HeadscaleTransport({
      type: 'headscale',
  ...
  describe('constructor', () => {
    it('should create transport with server URL', () => {
      const t = new HeadscaleTransport({
        type: 'headscale',
        serverUrl: 'https://headscale.example.com',
      })
      expect(t.type).toBe('headscale')
      expect(t.getServerUrl()).toBe('https://headscale.example.com')
    })
    ...
    it('should create transport with pre-auth key', () => {
      const t = new HeadscaleTransport({
        type: 'headscale',
        serverUrl: 'https://headscale.example.com',
        preAuthKey: 'tskey-abc123',
      })
      expect(t.getConfig().preAuthKey).toBe('tskey-abc123')
    })

    it('should create transport with API key', () => {
      const t = new HeadscaleTransport({
        type: 'headscale',
        serverUrl: 'https://headscale.example.com',
        apiKey: 'api-key-xyz',
      })
      expect(t.getConfig().apiKey).toBe('api-key-xyz')
    })
  })
  ...
  describe('start/stop lifecycle', () => {
    ...
    it('should throw if Headscale is not connected', async () => {
      const newTransport = new HeadscaleTransport({
        type: 'headscale',
        serverUrl: 'https://headscale.example.com',
        port: 19999,
      })
      ...
      await expect(newTransport.start()).rejects.toThrow('Headscale is not connected')
    })
  })
  ...
  describe('Headscale-specific methods', () => {
    beforeEach(async () => {
      await transport.start()
    })
    ...
    it('should return CLI instance', () => {
      ...
      expect(cli).toBeInstanceOf(HeadscaleCLI)
    })

    it('should get Headscale peers', async () => {
      const cli = transport.getCLI()
      vi.spyOn(cli, 'getPeers').mockResolvedValue([
        {
          hostname: 'peer-1',
      ...
      const peers = await transport.getHeadscalePeers()
      expect(peers).toHaveLength(1)
      expect(peers[0].hostname).toBe('peer-1')
    })

    it('should get online Headscale peers', async () => {
      const cli = transport.getCLI()
      vi.spyOn(cli, 'getOnlinePeers').mockResolvedValue([
        {
          hostname: 'peer-1',
      ...
      const peers = await transport.getOnlineHeadscalePeers()
      expect(peers).toHaveLength(1)
    ...
    it('should return server URL', () => {
      expect(transport.getServerUrl()).toBe('https://headscale.example.com')
    })
  })
...
describe('HeadscaleCLI', () => {
  it('should be exported from the module', () => {
    expect(HeadscaleCLI).toBeDefined()
  })
})
